...
namespace photo {
  ...
  __device__ __forceinline__ void computeAtxA(Matrix6d& dest, const Matrix5_6f& src, const float& lambda) {
    for (int c = 0; c < 6; ++c) {
      for (int r = 0; r <= c; ++r) {
  ...
  __device__ __forceinline__ Vector21d mat6DUpperTriangularToVector(const Matrix6d& mat) {
    Vector21d vec;
    int k = 0;
    for (int i = 0; i < 6; ++i) {
      for (int j = i; j < 6; ++j) {
  ...
  __device__ __forceinline__ void unpackIntFloat(int& a, float& b, unsigned long long val) {
