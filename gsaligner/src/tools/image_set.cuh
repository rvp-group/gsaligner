...
namespace photo {
  ...
  using MatrixCloud = DualMatrix_<Pointf>; // matrix vector cloud and matrix cloud can be taken from here
  ...
  getSubPixel(Vector5f& value, Matrix5_2f& derivative, const Image* mat, const Eigen::Vector2f& image_point) {
    ...
    derivative = (p00.derivatives * dc1 + p01.derivatives * dc) * dr1 + (p10.derivatives * dc1 + p11.derivatives * dc) * dr;
  ...
  public:
    ...
    void toCloud(MatrixCloud& target) const;                // puts a cloud rendered from self in target
    __host__ void toCloudDevice(MatrixCloud* target) const; // does the same but in device
    ...
    void fromCloud(const MatrixCloud& src_cloud); // fills own values from src_cloud
